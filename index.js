...
// =======================
// 3D Parallax Tilt Engine (Desktop Only)
// =======================
...
const certData = {
  ...
  "HackerRank": [
    { name: "JavaScript", file: "certificates/Hacker ranker.pdf#page=6" },
    { name: "Python", file: "certificates/Hacker ranker.pdf#page=1" },
    { name: "Java", file: "certificates/Hacker ranker.pdf#page=2" },
    { name: "SQL (Intermediate)", file: "certificates/Hacker ranker.pdf#page=3" },
    { name: "SQL (Advanced)", file: "certificates/Hacker ranker.pdf#page=4" }
  ],
...
function openParallaxView(companyName) {
  ...
  certs.forEach((cert, index) => {
    ...
    card.innerHTML = `
      <canvas id="${canvasId}" class="cert-canvas-img"></canvas>
      <div class="cert-bento-label">
        <span class="cert-index">${index + 1} / ${certs.length}</span>
        <span class="cert-name">${cert.name}</span>
        <a href="${fileUrl}" download class="cert-download-btn"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg></a>
      </div>
    `;
...
// PROFILE CARD 3D TILT ENGINE (Desktop Only)
if (window.innerWidth > 1024) {
...
window.addEventListener('load', () => {
  setTimeout(dismissPreloader, 1000); // 1s buffer after load
});

// Safety timeout: dismiss after 5s regardless of load state
...
if (contactForm) {
  contactForm.addEventListener('submit', async (e) => {
    ...
    formStatus.textContent = 'Launching your message...';
    formStatus.style.color = 'var(--primary-glow)';
